...
const URL_SEARCH_SUGGESTIONS =
  "https://suggestqueries-clients6.youtube.com/complete/search?client=youtube&gs_ri=youtube&ds=yt&q=";
...
const REGEX_VIDEO_URL_DESKTOP = new RegExp(
  "https://(.*\\.)?youtube\\.com/watch.*?v=(.*)"
);
const REGEX_VIDEO_URL_SHARE = new RegExp("https://youtu\\.be/(.*)");
const REGEX_VIDEO_URL_SHARE_LIVE = new RegExp(
  "https://(.*\\.)?youtube\\.com/live/(.*)"
);
const REGEX_VIDEO_URL_SHORT = new RegExp(
  "https://(.*\\.)?youtube\\.com/shorts/(.*)"
);
const REGEX_VIDEO_URL_CLIP = new RegExp(
  "https://(.*\\.)?youtube\\.com/clip/(.*)[?]?"
);
const REGEX_VIDEO_URL_EMBED = new RegExp(
  "https://(.*\\.)?youtube\\.com/embed/([^?]+)"
);

const REGEX_VIDEO_CHANNEL_URL = new RegExp(
  "https://(.*\\.)?youtube\\.com/channel/(.*)"
);
const REGEX_VIDEO_CHANNEL_URL2 = new RegExp(
  "https://(.*\\.)?youtube\\.com/user/.*"
);
const REGEX_VIDEO_CHANNEL_URL3 = new RegExp(
  "https://(.*\\.)?youtube\\.com/@.*"
);

const REGEX_VIDEO_PLAYLIST_URL = new RegExp(
  "https://(.*\\.)?youtube\\.com/playlist\\?list=.*"
);

const REGEX_INITIAL_DATA = new RegExp(
  "<script.*?var ytInitialData = (.*?);</script>"
);
const REGEX_INITIAL_PLAYER_DATA = new RegExp(
...
const REGEX_DATE_HUMAN = new RegExp(
  "([A-Za-z]*) ([0-9]*), ([1-9][0-9][0-9][0-9])"
);
const REGEX_DATE_ISO = new RegExp(
  "([1-9][0-9][0-9][0-9])-([0-9][0-9])-([0-9][0-9])"
);
const REGEX_DATE_EU = new RegExp(
  "([0-9][0-9])-([0-9][0-9])-([1-9][0-9][0-9][0-9])"
);
const REGEX_DATE_US = new RegExp(
  "([0-9][0-9])/([0-9][0-9])/([1-9][0-9][0-9][0-9])"
);
...
const REGEX_ASR = new RegExp(
  /<text .*?start="(.*?)" .*?dur="(.*?)".*?>(.*?)<\/text>/gms
);
...
const USER_AGENT_IOS =
  "com.google.ios.youtube/17.31.4(iPhone14,5; U; CPU iOS 15_6 like Mac OS X; US)";
const USER_AGENT_ANDROID =
  "com.google.android.youtube/17.31.35 (Linux; U; Android 12; US) gzip";
const USER_AGENT_TVHTML5_EMBED =
...
source.enable = (conf, settings, saveStateStr) => {
  ...
  try {
    if (saveStateStr) {
      ...
      if (saveState && saveState.clientContext && saveState.clientContextAuth) {
  ...
  if (!didSaveState) {
    ...
    if (isLoggedIn)
      batchReq = batchReq.GET(
        URL_CONTEXT_M,
        { "User-Agent": USER_AGENT_TABLET, "Accept-Language": "en-US" },
        true
      );
    ...
    if (!batchResp[0].isOk)
      throw new ScriptException(
        "Failed to request context enable !batchResp[0].isOk"
      );

    if (isLoggedIn && !batchResp[1].isOk)
      throw new ScriptException(
        "Failed to request context enable isLoggedIn && !batchResp[1].isOk"
      );
  ...
  if (IS_TESTING) console.log("Context", innerContext);
...
source.getHome = () => {
  ...
  else initialData = requestInitialData(URL_HOME, USE_MOBILE_PAGES, true);
  const tabs = extractPage_Tabs(initialData);
  if (tabs.length == 0) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getHome - No tabs found..",
  ...
  else return source.getTrending();
};
...
source.searchSuggestions = (query) => {
  const suggestionsResp = http.GET(
    URL_SEARCH_SUGGESTIONS +
      query +
      "&hl=" +
      langDisplay.toLowerCase() +
      "&gl=" +
      langRegion.toLowerCase(),
    {}
  );
  ...
  if (startIndex < 0)
    throw new ScriptException("Failed to filter suggestions (startIndex)");
  const endIndex = suggestionsRaw.lastIndexOf(")");
  if (endIndex < 0)
    throw new ScriptException("Failed to filter suggestions (endIndex)");
  const suggestions = JSON.parse(
    suggestionsRaw.substring(startIndex, endIndex)
  );
...
source.search = function (query, type, order, filters) {
  const param =
    order || (filters && Object.keys(filters).length > 0)
      ? searchQueryToSP(order, type, filters)
      : null;
  if (IS_TESTING && param) console.log("Search Param:", param);
...
source.searchChannelContents = function (
  ...
  if (tab) return new RichGridPager(tab, {}, USE_MOBILE_PAGES, true);
  else {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "searchChannelContents - No search tab found",
        JSON.stringify(initialData)
      );
...
source.getChannelUrlByClaim = (claimType, claimValues) => {
  ...
  if (atName) return URL_BASE + "/" + atName;
  else return URL_BASE + "/channel/" + values[0];
};
...
source.isContentDetailsUrl = (url) => {
  return (
    REGEX_VIDEO_URL_DESKTOP.test(url) ||
...
source.getContentDetails = (url, useAuth) => {
  ...
  if (IS_TESTING) console.log("VideoID:", videoId);
  ...
  const ageRestricted =
    initialPlayerData.playabilityStatus?.reason?.indexOf("your age") > 0 ??
    false;
  if (ageRestricted) {
    if (_settings["allowAgeRestricted"]) {
      const sts = _sts[jsUrl];
      if (!initialPlayerData.streamingData && sts) {
        initialPlayerData = requestTvHtml5EmbedStreamingData(
          initialPlayerData.videoDetails.videoId,
          sts
        );
      ...
      throw new AgeException(
        "Age restricted videos can be allowed using the plugin settings"
      );
    }
  }
  const controversial =
    initialPlayerData.playabilityStatus?.errorScreen?.playerErrorMessageRenderer?.reason?.simpleText?.indexOf(
      "following content may"
    ) > 0 ?? false;
  if (controversial) {
    if (_settings["allowControversialRestricted"]) {
      ...
      throw new UnavailableException(
        "Controversial restricted videos can be allowed using the plugin settings"
      );
    }
  }
  ...
  const videoDetails = extractVideoPage_VideoDetails(
    initialData,
    initialPlayerData,
    {
      url: url,
  ...
  if (videoDetails == null) throw new UnavailableException("No video found");

  if (
    !videoDetails.live &&
    (videoDetails.video?.videoSources == null ||
      videoDetails.video.videoSources.length == 0) &&
    (!videoDetails.datetime ||
      videoDetails.datetime < new Date().getTime() / 1000 - 60 * 60)
  ) {
    if (isNewCipher) {
  ...
  if (USE_ANDROID_FALLBACK && videoDetails.dash && videoDetails.dash.url) {
    ...
    if (IS_TESTING) console.log("Android Streaming Data", androidData);
  ...
  if (USE_IOS_FALLBACK && videoDetails.hls && videoDetails.hls.url) {
    ...
    if (IS_TESTING) console.log("IOS Streaming Data", iosData);
  ...
  if (resps.length > 1) {
    try {
      ...
      if (youtubeDislikeInfoResponse.isOk) {
        ...
        if (IS_TESTING) console.log("Youtube Dislike Info", youtubeDislikeInfo);
        videoDetails.rating = new RatingLikesDislikes(
          videoDetails.rating.likes,
          youtubeDislikeInfo.dislikes
        );
      }
    } catch (ex) {
...
source.getContentChapters = function (url, initialData) {
  ...
  if (initialData == null) {
    const reqs = http.batch().GET(url, getRequestHeaders({}), false);
    ...
    if (_settings["sponsorBlock"] && videoId) sbResp = resps[1];
  ...
  if (sbResp && sbResp.isOk) {
    try {
      ...
      const skipType = _settings["sponsorBlockType"]
        ? Type.Chapter.SKIP
        : Type.Chapter.SKIPPABLE;
      const sbData = JSON.parse(sbResp.body);
      for (let block of sbData) {
        if (
          block.actionType == "skip" &&
          block.segment &&
          block.segment.length == 2 &&
  ...
  try {
    videoChapters = videoChapters.concat(
      extractVideoChapters(initialData) ?? []
    );
  } catch (ex) {
    //Chapters failed
  }
  ...
  else if (videoChapters.length > 0) return videoChapters;
  else if (sbChapters.length > 0) return sbChapters;
  else return [];
};
function mergeSBChapters(videoChapters, sbChapters) {
  let newChapters = [];
  for (let videoChapter of videoChapters) {
    const sponsors = sbChapters.filter(
      (x) =>
    ...
    if (sponsors.length > 0) {
      ...
      for (let sponsorI = 0; sponsorI < sponsors.length && !skip; sponsorI++) {
        ...
        const nextSponsor =
          sponsorI + 1 < sponsors.length ? sponsors[sponsorI + 1] : null;
        ...
        const videoChapterAfter = {
          ...
          timeEnd:
            nextSponsor != null ? nextSponsor.timeStart : videoChapter.timeEnd,
          type: videoChapter.type,
        };

        if (
          sponsor.timeStart <= startTime &&
          sponsor.timeEnd <= videoChapter.timeEnd
        ) {
...
function extractVideoChapters(initialData) {
  ...
  if (rawObjects?.decoratedPlayerBarRenderer)
    rawObjects =
      rawObjects.decoratedPlayerBarRenderer?.playerBar
        ?.multiMarkersPlayerBarRenderer?.markersMap;
  else
    rawObjects =
      rawObjects.playerBar?.multiMarkersPlayerBarRenderer?.markersMap;

  if (!rawObjects || rawObjects.length == 0) return [];

  const chapters =
    rawObjects.find((x) => x.key == "DESCRIPTION_CHAPTERS") ??
    rawObjects.find((x) => x.key == "AUTO_CHAPTERS");
  if (chapters?.value?.chapters == null) return [];

  let result = [];
  const validChapters = chapters.value.chapters.filter(
    (x) =>
      x.chapterRenderer &&
      x.chapterRenderer.title &&
      (x.chapterRenderer.timeRangeStartMillis ||
        x.chapterRenderer.timeRangeStartMillis === 0)
  );
  for (let i = 0; i < validChapters.length; i++) {
    const chapter = validChapters[i]?.chapterRenderer;
    const chapterNext =
      i + 1 < validChapters.length
        ? validChapters[i + 1]?.chapterRenderer
        : null;

    const resultChapter = {
      name: extractText_String(chapter.title),
      timeStart: parseInt(chapter.timeRangeStartMillis / 1000),
      timeEnd: chapterNext?.timeRangeStartMillis
        ? parseInt(chapterNext.timeRangeStartMillis / 1000)
        : 999999, //Easier than re-parsing video end,
      type: Type.Chapter.NORMAL,
    };
...
function getVideoDetailsHtml(url, useLogin) {
  ...
  if (result.isOk) return result.body;
    ...
    throw new ScriptException(
      "Failed to get video details [" + url + "] (" + result.code + ")"
    );
}
source.getLiveChatWindow = function (url) {
  const id = extractVideoIDFromUrl(url);
  if (!id) throw new ScriptException("No valid id found");
    ...
    try {
      ...
      if (continuations) {
        const continuation = continuations.find(
          (x) => x.reloadContinuationData?.continuation
        );
        if (continuation) {
          chatUrl =
            URL_LIVE_CHAT_HTML +
      ...
      log(
        "Failed to get live chat window continuation, fallback to standard\n" +
          ex
      );
    }
  }
  ...
  if (!chatHtmlResp.isOk) return null;
  else {
    return {
      url: chatUrl,
...
source.getLiveEvents = function (url) {
  ...
  if (!chatHtmlResp.isOk) throw new ScriptException("Failed to get chat html");
...
source.getPlaybackTracker = function (url, initialPlayerData) {
  if (!_settings["youtubeActivity"] || !bridge.isLoggedIn()) return null;
...
class YoutubePlaybackTracker extends PlaybackTracker {
  constructor(playerData) {
    ...
    this.watchUrl =
      playerData.playbackTracking?.videostatsWatchtimeUrl?.baseUrl;
    this.playbackUrl =
      playerData.playbackTracking?.videostatsPlaybackUrl?.baseUrl;
    if (!this.playbackUrl || !this.watchUrl)
      throw new ScriptException("Playback tracking unavailable");

    this.playbackUrlBase = this.playbackUrl.substring(
      0,
      this.playbackUrl.indexOf("?")
    );
    ...
    if (this.playbackParams["plid"])
      // && !this.watchParams["plid"])
      this.watchParams["plid"] = this.playbackParams["plid"];
    if (this.playbackParams["of"])
      // && !this.watchParams["of"])
      this.watchParams["of"] = this.playbackParams["of"];
    if (this.playbackParams["vm"])
      // && !this.watchParams["vm"])
      this.watchParams["vm"] = this.playbackParams["vm"];
    if (this.playbackParams["ei"])
      // && !this.watchParams["ei"])
      this.watchParams["ei"] = this.playbackParams["ei"];
    if (this.playbackParams["cl"])
      // && !this.watchParams["cl"])
      this.watchParams["cl"] = this.playbackParams["cl"];

    const missing = ["plid", "of", "vm", "ei", "cl"].filter(
      (x) => !this.watchParams[x]
    );
    if (missing && missing.length > 0)
      throw new ScriptException(
        "Missing playback tracking parameter: " + missing.join(",")
      );
  }
  onInit(seconds) {
    //Initial playback
    const resp = http.GET(
      constructUrl(
        this.playbackUrlBase,
        this.getProgressParameters(this.playbackParams, seconds)
      ),
      getAuthContextHeaders(false),
      true
    );
    if (!resp.isOk)
  ...
  onProgress(seconds, isPlaying) {
    const resp = http.GET(
      constructUrl(
        this.watchUrlBase,
        this.getProgressParameters(this.watchParams, seconds, !isPlaying)
      ),
      getAuthContextHeaders(false),
      true
    );
  ...
  getProgressParameters(baseParameters, progress, paused) {
    ...
    if (!paused) q["rtn"] = position + 10; //Next RT
    else q["final"] = 1;
...
function constructUrl(base, queryParams) {
  ...
  for (let key in queryParams) {
    if (queryParams[key] === undefined) url += "";
    else if (!hasQ) {
      url += "?" + key + "=" + queryParams[key];
      hasQ = true;
    } else {
      if (queryParams[key] === "") url += "&" + key;
      else url += "&" + key + "=" + queryParams[key];
    }
  }
  return url;
}
...
source.getComments = (url) => {
  ...
  const result =
    contents.twoColumnWatchNextResults?.results?.results?.contents ?? null; //Add any alternative containers
  if (!result) return new CommentPager([], false);
...
source.getSubComments = (comment) => {
  ...
  if (comment.context?.replyContinuation) {
    return requestCommentPager(
      comment.contextUrl,
      comment.context.replyContinuation
    );
  } else return new CommentPager([], false);
};

//Channel
source.isChannelUrl = (url) => {
  return (
    REGEX_VIDEO_CHANNEL_URL.test(url) ||
...
source.getChannelContents = (url, type, order, filters) => {
  ...
  if (useAuth) log("USING AUTH FOR CHANNEL");
  ...
  const contextData = {
    authorLink: new PlatformAuthorLink(
      new PlatformID(PLATFORM, channel.id.value, config.id, PLATFORM_CLAIMTYPE),
      channel.name,
      channel.url,
      channel.thumbnail
    ),
  ...
  if (!tab) return new VideoPager([], false);
  if (IS_TESTING) console.log("Tab", tab);
  ...
  if (type == Type.Feed.Live) {
    if (tab.shelves) {
      ...
      if (featured && featured.videos && featured.videos.length > 0) {
        ...
        if (live) return new VideoPager([live], false);
      }
    }
...
source.peekChannelContents = function (url, type) {
  if (type != Type.Feed.Mixed && type != Type.Feed.Videos) return [];
  ...
  for (let entry of entryNodes) {
    ...
    videos.push(
      new PlatformVideo({
        id: new PlatformID(
          PLATFORM,
          entry.children.find((x) => x.name == "yt:videoid").value,
          config.id
        ),
        name: group.children.find((x) => x.name == "media:title").value,
        thumbnails: new Thumbnails([
          new Thumbnail(
            group.children.find((x) => x.name == "media:thumbnail")?.attributes[
              "url"
            ],
            1
          ),
        ...
        uploadDate: parseInt(
          new Date(
            entry.children.find((x) => x.name == "updated").value
          ).getTime() / 1000
        ),
        duration: 0,
        viewCount:
          parseInt(
            community.children.find((x) => x.name == "media:statistics")
              .attributes["views"]
          ) ?? 0,
...
source.searchPlaylists = function (query, type, order, filters) {
  ...
  if (IS_TESTING) console.log("Search data:", data);
  const searchResults = extractSearch_SearchResults(data);
...
source.getPlaylist = function (url) {
  ...
  if (IS_TESTING) console.log("Initial data", initialData);
  ...
  if (IS_TESTING) console.log("initialData", initialData);

  const renderer =
    initialData?.contents?.singleColumnBrowseResultsRenderer ??
    initialData?.contents?.twoColumnBrowseResultsRenderer;
  if (renderer) {
    if (!renderer.tabs) {
      log("No tabs found");
      return null;
    ...
    for (let playlistRenderer of playlistList.contents) {
      switchKey(playlistRenderer, {
        ...
        continuationItemRenderer(continueRenderer) {
          continuationToken =
            continueRenderer?.continuationEndpoint?.continuationCommand?.token;
        },
      });
    }
    ...
    while (continuationToken) {
      const newData = validateContinuation(() =>
        requestBrowse(
          {
      ...
      for (let playlistRenderer of newData) {
        switchKey(playlistRenderer, {
          ...
          continuationItemRenderer(continueRenderer) {
            continuationToken =
              continueRenderer?.continuationEndpoint?.continuationCommand
                ?.token;
          },
        });
      }
    ...
    if (
      ...
      thumbnail =
        videos[0].thumbnails.sources[videos[0].thumbnails.sources.length - 1]
          .url;
    ...
    return new PlatformPlaylistDetails({
      ...
      id: new PlatformID(
        PLATFORM,
        playlistHeaderRenderer?.playlistId,
        config.id
      ),
...
source.getUserPlaylists = function () {
  ...
  let subsPage = requestPage(
    URL_PLAYLISTS_M,
    { "User-Agent": USER_AGENT_PHONE },
    true
  );
  let result = getInitialData(subsPage);
  if (IS_TESTING) console.log("Initial Data", result);

  return switchKey(result.contents, {
    singleColumnBrowseResultsRenderer(renderer) {
      if (!renderer.tabs || renderer.tabs <= 0) return [];
      const tab = renderer.tabs[0];
      ...
      let playlistShelf = sections.find(
        (x) =>
          x.shelfRenderer &&
          extractText_String(x.shelfRenderer.title) == "Playlists"
      );

      let playlistItemSection =
        playlistShelf != null
          ? null
          : sections.find(
              (x) =>
                x.itemSectionRenderer &&
                x.itemSectionRenderer.contents &&
                x.itemSectionRenderer.contents.length > 0 &&
                x.itemSectionRenderer.contents[0].horizontalCardListRenderer &&
                x.itemSectionRenderer.contents[0].horizontalCardListRenderer
                  .cards &&
                extractText_String(
                  x.itemSectionRenderer.contents[0].horizontalCardListRenderer
                    .header?.richListHeaderRenderer?.title
                ) == "Playlists"
            );

      if (playlistShelf != null)
        playlistsItems =
          playlistShelf.shelfRenderer?.content?.verticalListRenderer?.items;
      else if (playlistItemSection != null)
        playlistsItems =
          playlistItemSection.itemSectionRenderer.contents[0]
      ...
      if (IS_TESTING) console.log("Playlist Items:", playlistsItems);

      let playlistUrls = [];
      for (let playlist of playlistsItems) {
        switchKey(playlist, {
          compactPlaylistRenderer(renderer) {
            const playlistUrl = extractNavigationEndpoint_Url(
              renderer.navigationEndpoint
            );
            if (playlistUrl) playlistUrls.push(playlistUrl);
          },
          ...
          for (let item of itemContents) {
            switchKey(item, {
              channelListItemRenderer(itemRenderer) {
                const authorUrl = extractNavigationEndpoint_Url(
                  itemRenderer.navigationEndpoint
                );
                if (authorUrl) subs.push(authorUrl);
              },
            });
          }
          return subs;
            ...
            for (let item of menuRenderer.contents) {
              switchKey(item, {
                channelListSubMenuAvatarRenderer(itemRenderer) {
                  const author =
                    extractChannelListSubMenuAvatarRenderer_URL(itemRenderer);
                  if (author) subs.push(author);
                },
                default(name) {
...
function extractVideoIDFromUrl(url) {
  ...
  if (match) return removeQuery(match[2]);

  match = url.match(REGEX_VIDEO_URL_SHARE);
  if (match) return removeQuery(match[1]);
...
class YTLiveEventPager extends LiveEventPager {
  ...
  nextPage() {
    const newResult = http.POST(
      URL_LIVE_CHAT + "?key=" + this.key + "&prettyPrint=false",
      JSON.stringify({
        context: {
          client: {
            clientName: "WEB",
    ...
    const continuationArr =
      json?.continuationContents?.liveChatContinuation?.continuations;
    if (!continuationArr || continuationArr.length == 0) {
      this.hasMore = false;
      throw new ScriptException("No chat continuation found");
    }
    const continuation =
      continuationArr[0]?.timedContinuationData?.continuation ??
      continuationArr[0]?.invalidationContinuationData?.continuation;
    if (!continuation) {
      this.hasMore = false;
      throw new ScriptException("No chat continuation found");
    }
    ...
    if (IS_TESTING) console.log("Live Chat Actions:", actions);
    let events = [];
    if (actions && actions.length > 0) {
      ...
      for (let kv in emojiMap) {
        if (this.knownEmojis[kv]) delete emojiMap[kv];
        else {
  ...
  for (let action of actions) {
    try {
      if (action.addChatItemAction) {
        ...
        const renderer = isPaid
          ? obj.item?.liveChatPaidMessageRenderer
          : obj.item?.liveChatTextMessageRenderer;
        ...
        if (msgObj && msgObj.name && (msgObj.message || isPaid)) {
          if (!isPaid)
            ...
            const amount = extractText_String(
              renderer.amount ??
                renderer.purchaseAmountText ??
                paidMessageRenderer?.amount ??
                paidMessageRenderer?.purchaseAmountText
            );
            events.push(
              new LiveEventDonation(
                amount,
                msgObj.name,
                msgObj.message ?? "",
                msgObj.thumbnail,
                ...
                renderer.bodyBackgroundColor
                  ? "#" + Number(renderer.bodyBackgroundColor).toString(16)
                  : null
              )
            );
          }
          ...
          if (msgObj && msgObj.name)
            events.push(
              new LiveEventDonation(
                ...
                msgObj.message,
                msgObj.thumbnail,
                (renderer.durationSec ?? 10) * 1000,
                membershipRenderer.bodyBackgroundColor
                  ? "#" +
                    Number(membershipRenderer.bodyBackgroundColor).toString(16)
                  : null
              )
            );
        } else if (obj.item?.liveChatTickerPaidMessageItemRenderer) {
          const renderer = obj.item?.liveChatTickerPaidMessageItemRenderer;
          const paidMessageRenderer =
            renderer.showItemEndpoint?.showLiveChatItemEndpoint?.renderer
          ...
          if (msgObj && msgObj.name)
            events.push(
              new LiveEventDonation(
                ...
                msgObj.thumbnail,
                (renderer.durationSec ?? 10) * 1000,
                paidMessageRenderer.bodyBackgroundColor
                  ? "#" +
                    Number(paidMessageRenderer.bodyBackgroundColor).toString(16)
                  : null
              )
            );
        }
        ...
        const bannerRenderer =
          action.addBannerToLiveChatCommand?.bannerRenderer
            ?.liveChatBannerRenderer;
        const redirectRenderer =
          bannerRenderer?.contents?.liveChatBannerRedirectRenderer;
        ...
        if (
          ...
          const url =
            redirectRenderer.inlineActionButton?.buttonRenderer?.command
              ?.commandMetadata?.webCommandMetadata?.url;
          const name = redirectRenderer.bannerMessage?.runs?.find(
            (x) => x.bold
          )?.text;
      ...
      log(
        "Failed Youtube live action parse due to [" +
          ex +
          "]: " +
          JSON.stringify(action, null, "   ")
      );
...
function extractLiveMessage_Obj(obj) {
  ...
  if (obj.authorBadges) {
    for (let badge of obj.authorBadges) {
      ...
      if (badgeImages && badgeImages.length > 0 && badgeName) {
        ...
        if (badgeName.toLowerCase().indexOf("member") >= 0) isMember = true;
      }
    }
  }
  ...
  if (obj?.message?.runs) {
    for (let part of obj?.message?.runs) {
      if (
        part.emoji?.image?.accessibility?.accessibilityData?.label &&
        part.emoji?.image?.thumbnails
      ) {
        const label =
          part.emoji?.image?.accessibility?.accessibilityData?.label;
...
class YTCommentPager extends CommentPager {
  ...
  nextPage() {
    if (!this.continuation) return new CommentPager([], false);
    return (
      requestCommentPager(this.context, this.continuation) ??
  ...
  nextPage() {
    ...
    if (this.continuation) {
      ...
      if (newData && newData.length > 0) {
        ...
        const newItemSection = extractRichGridRenderer_Shelves(
          fakeRichGrid,
          this.context
        );

        if (
          newItemSection.videos &&
          newItemSection.videos.length == 0 &&
          newItemSection.shelves &&
          newItemSection.shelves.length > 0
        ) {
          if (IS_TESTING)
            console.log(
              "No videos in root found, checking shelves",
              newItemSection
            );
          let vids = [];
        ...
        log(
          "Call [RichGridPager.nextPage] continuation gave no appended items, setting empty page with hasMore to false"
        );
    }
    this.hasMore = false;
    this.results = [];
...
class SearchItemSectionVideoPager extends VideoPager {
  constructor(itemSection) {
    super(
      itemSection.videos,
      itemSection.videos.length > 0 && !!itemSection.continuation
    );
  ...
  nextPage() {
    ...
    if (this.continuation) {
      ...
      if (continueItems.length > 0) {
        ...
        const newItemSection = extractSectionListRenderer_Sections(
          fakeSectionList,
          this.context
        );
        if (newItemSection.videos)
...
class SearchItemSectionChannelPager extends ChannelPager {
  constructor(itemSection) {
    super(
      itemSection.channels,
      itemSection.channels.length > 0 && !!itemSection.continuation
    );
    this.continuation = itemSection.continuation;
  }
...
class SearchItemSectionPlaylistPager extends ChannelPager {
  constructor(itemSection) {
    super(
      itemSection.playlists,
      itemSection.playlists.length > 0 && !!itemSection.continuation
    );
    this.continuation = itemSection.continuation;
  }
...
function requestGuide(pageId) {
  ...
  const url =
    URL_GUIDE +
    "?key=" +
    clientContext.INNERTUBE_API_KEY +
    "&prettyPrint=false";

  const res = http.POST(
    url,
    JSON.stringify(body),
    getAuthContextHeaders(false, "application/json"),
    true
  );
  if (res.code != 200) {
...
function requestNext(body, useAuth = false) {
  ...
  if (
    !clientContext ||
    !clientContext.INNERTUBE_CONTEXT ||
    !clientContext.INNERTUBE_API_KEY
  )
    throw new ScriptException("Missing client context");
  body.context = clientContext.INNERTUBE_CONTEXT;
  const url =
    URL_NEXT + "?key=" + clientContext.INNERTUBE_API_KEY + "&prettyPrint=false";
...
function requestBrowse(body, useMobile = false, useAuth = false, attempt = 0) {
  ...
  if (useMobile) headers["User-Agent"] = USER_AGENT_TABLET;
  headers["Content-Type"] = "application/json";

  const baseUrl = !useMobile ? URL_BROWSE : URL_BROWSE_MOBILE;
  const url =
    baseUrl + "?key=" + clientContext.INNERTUBE_API_KEY + "&prettyPrint=false";
...
function requestSearch(query, useAuth = false, params = null) {
  ...
  if (!resp.isOk)
    throw new ScriptException("Failed to search [" + resp.code + "]");
...
function requestInitialData(url, useMobile = false, useAuth = false) {
  ...
  if (resp.isOk) {
    ...
    if (html.indexOf('<form action="https://consent.youtube.com/save"') > 0) {
      ...
      const consentData =
        "gl=US&m=0&app=0&pc=yt&continue=" +
        encodeURIComponent(url) +
        "&x=6&bl=boq_identityfrontenduiserver_20231017.04_p0&hl=en&src=1&cm=2&set_eom=true";
      const respConsent = http.POST(
      ...
      if (respConsent.isOk) {
        ...
        if (
          respConsent.body.indexOf(
            '<form action="https://consent.youtube.com/save"'
          ) > 0
        )
          throw new CriticalException(
            "Failed to refuse Google consent [" + respConsent.code + "]"
          );
        else html = respConsent.body;
      } else
        throw new CriticalException(
          "Failed to refuse Google consent [" + resp.code + "]"
        );
    }
    ...
    throw new ScriptException(
      "Failed to request page [" + resp.code + "]\n" + url + "\n"
    );
}
function requestClientConfig(useMobile = false, useAuth = false) {
  ...
  const resp = http.GET(
    !useMobile ? URL_CONTEXT : URL_CONTEXT_M,
    headers,
    useAuth
  );
  if (!resp.isOk)
    throw new ScriptException("Failed to request context requestClientConfig");
...
function requestIOSStreamingData(videoId) {
  ...
  if (resp.isOk) return JSON.parse(resp.body);
  else return null;
}
...
function getInitialData(html, useAuth = false) {
  ...
  if (match) {
    ...
    if (
      clientContext?.INNERTUBE_CONTEXT &&
      !clientContext.INNERTUBE_CONTEXT.client.visitorData &&
      initialData.responseContext?.webResponseContextExtensionData?.ytConfigData
        ?.visitorData
    ) {
      clientContext.INNERTUBE_CONTEXT.client.visitorData =
        initialData.responseContext?.webResponseContextExtensionData?.ytConfigData?.visitorData;
      log(
        "Found new visitor (auth) data: " +
          clientContext.INNERTUBE_CONTEXT.client.visitorData
      );
    }
    return initialData;
...
function extractChannelListSubMenuAvatarRenderer_AuthorLink(renderer) {
  const thumbnail =
    renderer?.thumbnail?.thumbnails && renderer.thumbnail.thumbnails.length > 0
  ...
  if (!url || !name) return null;
    ...
    return new PlatformAuthorLink(
      new PlatformID(PLATFORM, null, config?.id, PLATFORM_CLAIMTYPE),
      name,
      url,
      thumbnail
    );
...
function extractChannelListSubMenuAvatarRenderer_URL(renderer) {
  const canonicalUrl = renderer?.navigationEndpoint?.browseEndpoint
    ?.canonicalBaseUrl
    ? URL_BASE + renderer.navigationEndpoint.browseEndpoint.canonicalBaseUrl
    : null;
              ...
              if (collapseRenderer.expandableItems?.length > 0) {
                for (let item of collapseRenderer.expandableItems) {
                  switchKey(item, {
                    guideEntryRenderer(guideEntryRenderer) {
                      channels.push(
                        extractGuideEntry_AuthorLink(guideEntryRenderer)
                      );
                    },
                  });
                }
...
function extractGuideEntry_AuthorLink(guideEntryRenderer) {
  const thumbnail =
    guideEntryRenderer.thumbnail?.thumbnails?.length > 0
      ? guideEntryRenderer.thumbnail.thumbnails[0].url
      : null;
  const name =
    guideEntryRenderer.formattedTitle?.simpleText ??
    guideEntryRenderer.accessibility?.accessibilityData?.label;
  const url = guideEntryRenderer.navigationEndpoint?.browseEndpoint
    ?.canonicalBaseUrl
    ? URL_BASE +
      guideEntryRenderer.navigationEndpoint?.browseEndpoint?.canonicalBaseUrl
    : null;
...
function extractSearch_SearchResults(data, contextData) {
  let searchContents =
    data.contents?.twoColumnSearchResultsRenderer?.primaryContents
      ?.sectionListRenderer ?? data.contents?.sectionListRenderer;

  if (searchContents) {
    const results = extractSectionListRenderer_Sections(
      searchContents,
      contextData
    );
    return results;
  ...
  if (initialData?.header?.c4TabbedHeaderRenderer) {
    ...
    const thumbnail =
      thumbnails && thumbnails.length > 0
        ? thumbnails.sort(
            (a, b) =>
              Math.abs(a.width - thumbnailTargetWidth) -
              Math.abs(b.width - thumbnailTargetWidth)
          )[0]
        : { url: "" };
    ...
    const banner =
      banners && banners.length > 0
        ? banners.sort(
            (a, b) =>
              Math.abs(a.width - bannerTargetWidth) -
              Math.abs(b.width - bannerTargetWidth)
          )[0]
        : { url: "" };
    ...
    const idUrl = headerRenderer?.navigationEndpoint?.browseEndpoint?.browseId
      ? URL_BASE +
        "/channel/" +
        headerRenderer.navigationEndpoint.browseEndpoint.browseId
      : null;
    const canonicalUrl = headerRenderer?.navigationEndpoint?.browseEndpoint
      ?.canonicalBaseUrl
      ? URL_BASE +
        headerRenderer.navigationEndpoint.browseEndpoint.canonicalBaseUrl
      : null;
    ...
    return new PlatformChannel({
      ...
      subscribers: Math.max(
        0,
        extractHumanNumber_Integer(
          extractText_String(headerRenderer.subscriberCountText)
        )
      ),
      description: "",
      url: idUrl,
    ...
    if (IS_TESTING) console.log("Initial Data (New Model)", initialData);

    const thumbnailTargetWidth = 200;
    const thumbnails =
      headerRenderer?.content?.pageHeaderViewModel?.image
    ...
    const thumbnail =
      thumbnails && thumbnails.length > 0
        ...
        : { url: "" };
    const banners =
      headerRenderer?.content?.pageHeaderViewModel?.banner?.imageBannerViewModel
        ?.image?.sources;
    const bannerTargetWidth = 1080;
    const banner =
      banners && banners.length > 0
    ...
    if (!id) {
      log(
        "ID not found in new channel viewmodel:" +
          JSON.stringify(id, null, "   ")
      );
      if (bridge.devSubmit)
        bridge.devSubmit(
          "extractChannel_PlatformChannel - ID Not found in new channel view model",
          JSON.stringify(initialData)
        );
      throw new ScriptException("ID Not found in new channel view model");
    }

    const idUrl = id ? URL_BASE + "/channel/" + id : null;
    const canonicalUrl = initialData?.metadata?.channelMetadataRenderer
      ?.vanityChannelUrl
    ...
    let subCount = 0;
    const metadataRows =
      headerRenderer?.content?.pageHeaderViewModel?.metadata
        ?.contentMetadataViewModel?.metadataRows;
    for (let row of metadataRows) {
      const subsStr = row.metadataParts.find(
        (x) => x.text?.content?.indexOf("subscribers") > 0
      )?.text?.content;
      if (!subsStr) continue;
    ...
    if (bridge.devSubmit)
      bridge.devSubmit(
        "extractChannel_PlatformChannel - No header for " + sourceUrl,
        JSON.stringify(initialData)
      );
...
function extractPage_Tabs(initialData, contextData) {
  ...
  if (!content) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "extractPage_Tabs - Missing contents",
        JSON.stringify(initialData)
      );
  ...
  return switchKey(content, {
    ...
    singleColumnBrowseResultsRenderer(renderer) {
      return extractSingleColumnBrowseResultsRenderer_Tabs(
        renderer,
        contextData
      );
    },
    default(name) {
      if (bridge.devSubmit)
        bridge.devSubmit(
          "extractPage_Tabs - Unknown renderer type: " + name,
...
function extractVideoPage_VideoDetails(
  initialData,
  initialPlayerData,
  contextData,
  jsUrl
  ...
  const contentsContainer =
    contents.twoColumnWatchNextResults?.results?.results ?? null;
  if (
    !contentsContainer ||
    !contentsContainer.contents ||
    !initialPlayerData.videoDetails
  )
    return null;
  ...
  const video = {
    ...
    thumbnails: new Thumbnails(
      videoDetails.thumbnail?.thumbnails.map(
        (x) => new Thumbnail(x.url, x.height)
      ) ?? []
    ),
    author: new PlatformAuthorLink(
      ...
      videoDetails.author,
      URL_BASE + "/channel/" + videoDetails.channelId,
      null,
      null
    ),
    ...
    video: initialPlayerData?.streamingData?.adaptiveFormats
      ? new UnMuxVideoSourceDescriptor(
          initialPlayerData.streamingData.adaptiveFormats
            .filter((x) => x.mimeType.startsWith("video/"))
            .map((y) => {
              const codecs = y.mimeType
                .substring(y.mimeType.indexOf('codecs="') + 8)
                .slice(0, -1);
              const container = y.mimeType.substring(
                0,
                y.mimeType.indexOf(";")
              );
              if (codecs.startsWith("av01")) return null; //AV01 is unsupported.
              ...
              let url =
                decryptUrlN(y.url, jsUrl, logItag) ??
                decryptUrl(y.cipher, jsUrl, logItag) ??
                decryptUrl(y.signatureCipher, jsUrl, logItag);
              if (url.indexOf("&cpn=") < 0) url = url + "&cpn=" + nonce;

              const duration =
                parseInt(parseInt(y.approxDurationMs) / 1000) ?? 0;
              if (isNaN(duration)) return null;

              if (!y.initRange?.end || !y.indexRange?.end) return null;
          ...
          initialPlayerData.streamingData.adaptiveFormats
            ...
            .map((y) => {
              ...
              let url =
                decryptUrlN(y.url, jsUrl) ??
                decryptUrl(y.cipher, jsUrl) ??
                decryptUrl(y.signatureCipher, jsUrl);
              if (url.indexOf("&cpn=") < 0) url = url + "&cpn=" + nonce;
              ...
              return new YTAudioSource({
                name: y.audioTrack?.displayName
                  ? y.audioTrack.displayName
                  : codecs,
                container: container,
                bitrate: y.bitrate,
                url: url,
    ...
    subtitles:
      initialPlayerData.captions?.playerCaptionsTracklistRenderer?.captionTracks
        ?.map((x) => {
  ...
  if (
    video.video.videoSources !== null &&
    video.video.videoSources.length == 0 &&
    initialPlayerData?.streamingData?.hlsManifestUrl
  )
    video.video.videoSources.push(
      new HLSSource({ url: initialPlayerData.streamingData.hlsManifestUrl })
    );
    ...
    switchKey(content, {
      videoPrimaryInfoRenderer(renderer) {
        ...
        if (renderer.viewCount?.videoViewCountRenderer?.viewCount?.simpleText)
          video.viewCount = extractFirstNumber_Integer(
            renderer.viewCount?.videoViewCountRenderer?.viewCount.simpleText
          );
        else if (renderer.viewCount?.videoViewCountRenderer?.viewCount?.runs) {
          video.viewCount = parseInt(
            extractFirstNumber_Integer(
              extractRuns_String(
                renderer.viewCount?.videoViewCountRenderer?.viewCount?.runs
              )
            )
          );
        }
        if (
          renderer.viewCount?.videoViewCountRenderer?.isLive ||
          renderer.viewCount?.videoViewCountRenderer?.viewCount?.isLive
        )
          video.isLive = true;
        else video.isLive = false;

        if (renderer.videoActions?.menuRenderer?.topLevelButtons)
          renderer.videoActions.menuRenderer.topLevelButtons.forEach(
            (button) => {
              switchKey(button, {
                segmentedLikeDislikeButtonRenderer(renderer) {
                  const likeButtonRenderer =
                    renderer?.likeButton?.toggleButtonRenderer;
                  if (likeButtonRenderer) {
                    const likeTextData = likeButtonRenderer.defaultText;
                    if (likeTextData) {
                      if (likeTextData.accessibility?.accessibilityData?.label)
                        video.rating = new RatingLikes(
                          extractFirstNumber_Integer(
                            likeTextData.accessibility.accessibilityData.label
                          )
                        );
                      else if (likeTextData.simpleText)
                        video.rating = new RatingLikes(
                          extractHumanNumber_Integer(likeTextData.simpleText)
                        );
                    }
                  }
                },
                segmentedLikeDislikeButtonViewModel(renderer) {
                  ...
                  if (likeButtonViewModel.likeButtonViewModel)
                    //Youtube double nested, not sure if a bug on their end which may be removed
                    likeButtonViewModel =
                      likeButtonViewModel.likeButtonViewModel;
                  let toggleButtonViewModel =
                    likeButtonViewModel?.toggleButtonViewModel;
                  if (toggleButtonViewModel.toggleButtonViewModel)
                    //Youtube double nested, not sure if a bug on their end which may be removed
                  ...
                  if (buttonViewModel?.title) {
                    ...
                    if (!isNaN(num)) video.rating = new RatingLikes(num);
                    ...
                    if (!isNaN(num) && num >= 0)
                      ...
                      if (bridge.devSubmit)
                        bridge.devSubmit(
                          "extractVideoPage_VideoDetails - Found unknown likes model",
                          JSON.stringify(buttonViewModel)
                        );
                      throw new ScriptException(
                        "Found unknown likes model, please report to dev:\n" +
        ...
        if (!video.datetime || video.datetime <= 0) {
          ...
          if (date <= 0 && renderer.relativeDateText?.simpleText)
            date = extractAgoText_Timestamp(
              renderer.relativeDateText.simpleText
            );
      ...
      videoSecondaryInfoRenderer(renderer) {
        if (renderer.owner.videoOwnerRenderer)
          video.author = extractVideoOwnerRenderer_AuthorLink(
            renderer.owner.videoOwnerRenderer
          );
  ...
  const scheduledTime =
    initialPlayerData?.playabilityStatus?.liveStreamability
  ...
  result.getComments = function () {
    return extractTwoColumnWatchNextResultContents_CommentsPager(
      contextData.url,
      contentsContainer?.contents
    );
  };
...
function extractVideoOwnerRenderer_AuthorLink(renderer) {
  ...
  if (renderer.subscriberCountText)
    subscribers = extractHumanNumber_Integer(
      extractText_String(renderer.subscriberCountText)
    );
...
function extractTwoColumnWatchNextResultContents_CommentsPager(
  contextUrl,
  contents
) {
  //Add additional/better details
  ...
  if (!commentsToken) return new CommentPager([], false);
  return (
    requestCommentPager(contextUrl, commentsToken) ??
    new CommentPager([], false)
  );
}
function requestCommentPager(contextUrl, continuationToken) {
  ...
  if (IS_TESTING) console.log("data", data);
  const endpoints =
  ...
  if (!endpoints) {
    ...
    if (bridge.devSubmit)
      bridge.devSubmit(
        "requestCommentPager - No comment endpoints",
        JSON.stringify(data)
      );
        ...
        for (let continuationItem of continuationItems) {
          switchKey(continuationItem, {
            commentThreadRenderer(renderer) {
              ...
              const replyCount = commentRenderer.replyCount
                ? commentRenderer?.replyCount
                : 0;
              const replyContinuation =
                renderer.replies?.commentRepliesRenderer?.contents?.length == 1
                  ? renderer.replies.commentRepliesRenderer.contents[0]
                      ?.continuationItemRenderer?.continuationEndpoint
                      ?.continuationCommand?.token
                  : null;

              comments.push(
                extractCommentRenderer_Comment(
                  contextUrl,
                  commentRenderer,
                  replyCount,
                  replyContinuation
            ...
            commentRenderer(renderer) {
              comments.push(
                extractCommentRenderer_Comment(contextUrl, renderer, 0, null)
              );
            },
  ...
  if (data?.frameworkUpdates?.entityBatchUpdate?.mutations) {
    ...
    if (mutations.length > 0) {
      ...
      for (let i = 0; i < endpoints.length; i++)
        parentItems.push(
          ...(endpoints[i].reloadContinuationItemsCommand?.continuationItems ??
            endpoints[i].appendContinuationItemsAction?.continuationItems ??
            [])
        );
      parentItems = parentItems.filter((x) => x.commentThreadRenderer);
      const commentObjects = mutations.filter(
        (x) => x?.payload?.commentEntityPayload
      );
      ...
      for (let commentObject of commentObjects) {
        ...
        const parent = parentItems.find(
          (x) =>
            x.commentThreadRenderer?.commentViewModel?.commentViewModel
              ?.commentKey == commentObject.entityKey
        );
        const replyContents =
          parent?.commentThreadRenderer?.replies?.commentRepliesRenderer
            ?.contents;
        const replyContinuation =
          (replyContents?.length ?? 0) > 0
            ? replyContents[0].continuationItemRenderer?.continuationEndpoint
                ?.continuationCommand?.token
            : null;

        const authorEndpoint =
          cobj.author?.channelCommand?.innertubeCommand?.commandMetadata
        ...
        comments.push(
          new YTComment({
            ...
            author: new PlatformAuthorLink(
              new PlatformID(PLATFORM, null, config.id, PLATFORM_CLAIMTYPE),
              cobj.author.displayName,
              authorEndpoint ? URL_BASE + authorEndpoint : "",
              cobj.author.avatarThumbnailUrl
            ),
            message: cobj.properties?.content?.content ?? "",
            rating: new RatingLikes(
              extractHumanNumber_Integer(cobj.toolbar?.likeCountLiked) ?? 0
            ),
            date:
              extractAgoTextRuns_Timestamp(cobj?.properties?.publishedTime) ??
              0,
            replyCount:
              extractFirstNumber_Integer(cobj?.toolbar?.replyCount) ?? 0,
...
function extractSingleColumnBrowseResultsRenderer_Tabs(renderer, contextData) {
  ...
  if (!renderer.tabs) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "extractSingleColumnBrowseResultsRenderer_Tabs - No tabs found",
  ...
  for (let i = 0; i < renderer.tabs.length; i++) {
    ...
    switchKey(content, {
      richGridRenderer(renderer) {
        tabResult = extractRichGridRenderer_Shelves(
          content[tabContentRendererName],
          contextData
        );
      },
      ...
      default() {
        if (bridge.devSubmit)
          bridge.devSubmit(
            "extractSingleColumnBrowseResultsRenderer_Tabs - Unknown tab renderer: " +
              tabContentRendererName,
            JSON.stringify(content)
          );
        throw new ScriptException(
          "Unknown tab renderer: " + tabContentRendererName
        );
      },
    });
    if (tabResult) {
...
function extractTwoColumnBrowseResultsRenderer_Tabs(renderer, contextData) {
  ...
  if (!renderer.tabs) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "extractTwoColumnBrowseResultsRenderer_Tabs - No tabs found",
        JSON.stringify(renderer)
      );
    throw new ScriptException("No tabs found");
  }
  ...
  for (let i = 0; i < renderer.tabs.length; i++) {
    ...
    if (!tab.tabRenderer && !tab.expandableTabRenderer) continue;
    ...
    switchKey(content, {
      ...
      default() {
        if (bridge.devSubmit)
          bridge.devSubmit(
            "extractTwoColumnBrowseResultsRenderer_Tabs - Unknown tab renderer: " +
              tabContentRendererName,
            JSON.stringify(renderer)
          );
        throw new ScriptException(
  ...
  for (let ci = 0; ci < contents.length; ci++) {
    ...
    switchKey(content, {
      ...
      itemSectionRenderer(renderer) {
        ...
        if (items.shelves) shelves = shelves.concat(items.shelves);
        if (items.videos) videos = videos.concat(items.videos);
      },
      default(name) {
        log(
          "Unknown shelf/section renderer in extractRichGridRenderer_Shelves: " +
            name
        );
      },
    });
...
function extractSectionListRenderer_Sections(sectionListRenderer, contextData) {
  ...
  for (let i = 0; i < contents.length; i++) {
    ...
    switchKey(item, {
      itemSectionRenderer(renderer) {
        const items = extractItemSectionRenderer_Shelves(renderer);
        if (items.videos.length > 0) videos.push(...items.videos);
        if (items.channels.length > 0) channels.push(...items.channels);
        if (items.playlists.length > 0) playlists.push(...items.playlists);
        if (items.shelves) shelves.push(...items.shelves);
      },
      continuationItemRenderer(renderer) {
        continuation = extractContinuationItemRenderer_Continuation(
          renderer,
...
function extractContinuationItemRenderer_Continuation(
...
function extractRichSectionRenderer_Shelf(sectionRenderer, contextData) {
  ...
  return switchKey(content, {
    ...
    default(name) {
      log(
        "Unknown shelf renderer in extractRichSectionRenderer_Shelf: " + name
      );
      return null;
...
function extractRichShelfRenderer_Shelf(shelfRenderer, contextData) {
  ...
  for (let itemi = 0; itemi < shelfRenderer.contents.length; itemi++) {
    ...
    switchKey(item, {
      ...
      default(name) {
        log(
          "Unknown shelf renderer in extractRichShelfRenderer_Shelf: " + name
        );
...
function extractVideoWithContextRenderer_Video(videoRenderer, contextData) {
  const liveBadges = videoRenderer.thumbnailOverlays?.filter(
    (x) =>
      x.thumbnailOverlayTimeStatusRenderer?.accessibility?.accessibilityData
        ?.label == "LIVE"
  );
  ...
  const author =
    contextData && contextData.authorLink
      ? contextData.authorLink
      : extractVideoWithContextRenderer_AuthorLink(videoRenderer);

  if (IS_TESTING) console.log(videoRenderer);

  if (
    !videoRenderer?.lengthText?.runs ||
    !videoRenderer.publishedTimeText?.runs
  )
    isLive = true; //If no length, live after all?

  let viewCount = 0;
  if (videoRenderer?.shortViewCountText?.runs != null)
    viewCount = extractHumanNumber_Integer(
      extractRuns_String(videoRenderer.shortViewCountText.runs)
    );
  else log("No viewcount found on video " + videoRenderer.videoId);

  const title = videoRenderer.headline
    ? extractText_String(videoRenderer.headline)
    : extractText_String(videoRenderer.title);
  ...
  if (isLive) {
    ...
    return new PlatformVideo({
      ...
      author: author,
      uploadDate: parseInt(
        extractAgoText_Timestamp(
          extractText_String(videoRenderer.publishedTimeText)
        )
      ),
      duration: extractHumanTime_Seconds(
        extractText_String(videoRenderer.lengthText)
      ),
      viewCount: viewCount,
      url: URL_BASE + "/watch?v=" + videoRenderer.videoId,
      isLive: false,
...
function extractVideoRenderer_Video(videoRenderer, contextData) {
  const liveBadges = videoRenderer.badges?.filter(
    (x) => x.metadataBadgeRenderer?.label == "LIVE"
  );
  const liveOverlays = videoRenderer.thumbnailOverlays?.filter(
    (x) =>
  ...
  let isLive =
    (liveBadges != null && liveBadges.length > 0) ||
    (liveOverlays != null && liveOverlays.length > 0);

  let plannedDate = null;
  if (videoRenderer.upcomingEventData?.startTime)
    plannedDate = parseInt(videoRenderer.upcomingEventData.startTime);
  if (plannedDate) isLive = true;

  if (!isLive && !videoRenderer.publishedTimeText?.simpleText) return null; //Not a normal video

  const author =
    contextData && contextData.authorLink
      ? contextData.authorLink
      : extractVideoRenderer_AuthorLink(videoRenderer);

  if (IS_TESTING) console.log(videoRenderer);

  if (!videoRenderer?.lengthText?.simpleText) isLive = true; //If no length, live after all?
  ...
  if (isLive)
    return new PlatformVideo({
      ...
      uploadDate: plannedDate ?? parseInt(new Date().getTime() / 1000),
      duration: 0,
      viewCount: !videoRenderer.viewCountText
        ? 0
        : extractRuns_ViewerCount(videoRenderer.viewCountText.runs),
      url: URL_BASE + "/watch?v=" + videoRenderer.videoId,
      isLive: true,
      extractType: "Video",
    });
    ...
    return new PlatformVideo({
      ...
      author: author,
      uploadDate: parseInt(
        extractAgoText_Timestamp(videoRenderer.publishedTimeText.simpleText)
      ),
      duration: extractHumanTime_Seconds(videoRenderer.lengthText.simpleText),
      viewCount: extractFirstNumber_Integer(
        extractText_String(videoRenderer.viewCountText)
      ),
      url: URL_BASE + "/watch?v=" + videoRenderer.videoId,
      isLive: false,
      extractType: "Video",
...
function extractPlaylistVideoRenderer_Video(videoRenderer, contextData) {
  if (!videoRenderer.lengthText) return null;

  const author =
    contextData && contextData.authorLink
      ? contextData.authorLink
      : extractRuns_AuthorLink(videoRenderer.shortBylineText?.runs);
...
function extractPlaylistRenderer_Playlist(playlistRenderer, contextData) {
  const author =
    contextData && contextData.authorLink
      ? contextData.authorLink
      : extractRuns_AuthorLink(playlistRenderer.shortBylineText?.runs);
  ...
  return new PlatformPlaylist({
    ...
    thumbnail:
      playlistRenderer.thumbnails && playlistRenderer.thumbnails.length > 0
        ? extractThumbnail_BestUrl(playlistRenderer.thumbnails[0])
        : null,
    url: URL_PLAYLIST + playlistRenderer.playlistId,
    videoCount: extractFirstNumber_Integer(
      extractText_String(playlistRenderer.videoCountText)
    ),
  });
}
...
function extractChannelRenderer_AuthorLink(channelRenderer) {
  ...
  const channelUrl = extractNavigationEndpoint_Url(
    channelRenderer.navigationEndpoint
  );
  let thumbUrl = extractThumbnail_BestUrl(channelRenderer.thumbnail);
  if (thumbUrl.startsWith("//")) thumbUrl = "https:" + thumbUrl;

  const subscribers = extractHumanNumber_Integer(
    extractText_String(channelRenderer.videoCountText)
  );
...
function extractThumbnail_Thumbnails(thumbnail) {
  return new Thumbnails(
    thumbnail.thumbnails.map((x) => new Thumbnail(x.url, x.height))
  );
}
function extractThumbnail_BestUrl(thumbnail) {
  if (!thumbnail?.thumbnails || thumbnail.thumbnails.length <= 0) return null;
...
function extractVideoWithContextRenderer_AuthorLink(videoRenderer) {
  let id =
    videoRenderer.channelThumbnail?.channelThumbnailWithLinkRenderer
      ?.navigationEndpoint?.browseEndpoint?.browseId;
  const name = extractRuns_String(videoRenderer.shortBylineText.runs);
  const channelThumbs =
    videoRenderer.channelThumbnail.channelThumbnailWithLinkRenderer?.thumbnail
      ?.thumbnails;
  const thumbUrl =
    channelThumbs && channelThumbs.length > 0 ? channelThumbs[0].url : null;
  let channelUrl =
    videoRenderer.channelThumbnail?.channelThumbnailWithLinkRenderer
...
function extractVideoRenderer_AuthorLink(videoRenderer) {
  ...
  const channelUrl = !id
    ? extractRuns_Url(videoRenderer.ownerText.runs)
    : URL_BASE + "/channel/" + id;
...
function extractCommentRenderer_Comment(
  contextUrl,
  commentRenderer,
  replyCount,
  replyContinuation
  ...
  const authorEndpoint =
    commentRenderer.authorEndpoint?.commandMetadata?.webCommandMetadata?.url ??
    "";
  ...
  return new YTComment({
    ...
    rating: new RatingLikes(
      commentRenderer?.voteCount?.simpleText
        ? extractHumanNumber_Integer(commentRenderer.voteCount.simpleText)
        : 0
    ),
    date: commentRenderer.publishedTimeText?.runs
      ? extractAgoTextRuns_Timestamp(commentRenderer.publishedTimeText.runs)
      : 0,
...
function extractText_String(item) {
  ...
  if (item?.runs) return extractRuns_String(item.runs);

  if (item) log("Unknown string object: " + JSON.stringify(item, null, "   "));
  return null;
}
function extractRuns_String(runs) {
  ...
  for (let runi = 0; runi < runs.length; runi++) {
    const run = runs[runi];
    if (run.text) str += run.text;
    else if (run.emoji?.image?.accessibility?.accessibilityData?.label)
      str +=
        "__" + run.emoji?.image?.accessibility?.accessibilityData?.label + "__";
  }
  return str;
}
...
function extractNavigationEndpoint_Url(navEndpoint, baseUrl) {
  if (!baseUrl) baseUrl = URL_BASE;
  if (!navEndpoint) return null;
  if (
    navEndpoint?.browseEndpoint?.browseId &&
    navEndpoint?.browseEndpoint?.canonicalBaseUrl &&
    navEndpoint.browseEndpoint.canonicalBaseUrl.startsWith("/@")
  )
...
function extractAgoText_Timestamp(str) {
  ...
  switch (match[2]) {
    ...
    default:
      if (bridge.devSubmit)
        bridge.devSubmit(
          "extractAgoText_Timestamp - Unknown time type: " + match[2],
          match[2]
        );
...
function extractHumanTime_Seconds(str) {
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    if (isNaN(parts[i])) return seconds;
...
function extractHumanNumber_Integer(str) {
  ...
  if (!match) return extractFirstNumber_Integer(str);
...
function extractDate_Timestamp(dateStr) {
  if (!dateStr) return -1;
  if (dateStr.indexOf("ago") > 0) return extractAgoText_Timestamp(dateStr);
...
function extractHumanDate_Timestamp(dateParts) {
  ...
  for (let i = 0; i < dateParts.length; i++) {
    ...
    if (part.length > 2) {
      ...
      if (newMonth > 0) month = newMonth;
    }
    if (part.length == 4 && !isNaN(part)) year = parseInt(part);
    if (part.length <= 2 && !isNaN(part)) day = parseInt(part);
  }
...
const FILTERS = [
  ...
  },
  {
    ...
    filters: [
      new FilterCapability(
        "Under 4 minutes",
        FILTER_DURATION_4MIN,
        Type.Duration.Short
      ),
      new FilterCapability(
        "4-20 minutes",
        FILTER_DURATION_4_20MIN,
        Type.Duration.Medium
      ),
      new FilterCapability(
        "Over 20 minutes",
        FILTER_DURATION_20MIN,
        Type.Duration.Long
      ),
    ],
  },
...
function searchQueryToSP(sort, type, filters) {
  ...
  let filter_date =
    filters?.date && filters.date.length > 0 ? filters.date[0] : null;
  let filter_duration =
...
function decodeHtml(text) {
  return text
    .replace(/(?:&amp;|&)#([0-9]*);/gm, function (match, dec) {
      return String.fromCharCode(dec);
    })
    .replace(/&([a-z]*);(#.*?;)?/gm, function (match, c) {
      if (htmlEncodedCharacters[c]) return htmlEncodedCharacters[c];
      return c;
    });
}
...
function ytLangIdToLanguage(id) {
  if (!id) return Language.UNKNOWN;
  const langParts = id?.split(".");
  let langPart = langParts && langParts.length > 0 ? langParts[0] : "";
  if (ytLangMap[langPart]) return ytLangMap[langPart]; //Backwards compat
  if (langPart.indexOf("-") > 0) langPart = langPart.split("-")[0].trim();
  if (ytLangMap[langPart]) return ytLangMap[langPart]; //Backwards compat
  if (langPart && langPart.length > 0) return langPart.trim();
...
function findRenderer(obj, rendererName) {
  ...
  if (objName == rendererName) return renderer;
  ...
  if (renderer.content) return findRenderer(renderer.content, rendererName);
  return null;
}
...
function switchKey(obj, handlers) {
  ...
  if (!objName) {
    if (handlers["null"]) return handlers["null"];
    return null;
  }

  if (handlers[objName]) return handlers[objName](obj[objName]);
  if (handlers["default"]) return handlers["default"](objName);
  return null;
}
...
function validateContinuation(reqcb, useAuth = false) {
  ...
  const result = reqcb();
  const append =
    result?.onResponseReceivedCommands ?? result?.onResponseReceivedActions;
  if (append && append.length > 0 && append[0].appendContinuationItemsAction) {
    const appendResults =
      append[0].appendContinuationItemsAction.continuationItems;
    if (!appendResults) {
      if (IS_TESTING) console.log("Continuation found without items?", result);
      return [];
    } else return appendResults;
  } else if (
    !clientContext.INNERTUBE_CONTEXT.client.visitorData &&
    result.responseContext?.visitorData
  ) {
    log(
      "[validateContinuation] No visitor data set, found visitor data in response, retrying"
    );
    clientContext.INNERTUBE_CONTEXT.client.visitorData =
      result.responseContext.visitorData;
    ...
    if (
      append &&
      append.length > 0 &&
      append[0].appendContinuationItemsAction
    ) {
      const appendResults =
        append[0].appendContinuationItemsAction.continuationItems;
      if (!appendResults) {
...
source.decryptUrlTest = function (encrypted) {
  ...
  if (!url) url = decryptUrl(encrypted.cipher, true);
  if (!url) url = decryptUrl(encrypted.signatureCipher, true);
  return url;
};
...
function decryptUrl(encrypted, jsUrl, doLogging) {
  ...
  let decryptedUrl =
    decodeURIComponent(baseUrl) + "&" + sigKey + "=" + sigValue;
...
function decryptUrlN(url, jsUrl, doLogging) {
  ...
  if (nParamMatch) {
    ...
    if (doLogging) {
      log("Encrypt URL:" + url);
      log(
        "NParam Found: " + encryptedN + " (length:" + encryptedN.length + ")"
      );
      log(
        "NParam Decrypted: " + decryptedN + " (size:" + decryptedN.length + ")"
      );
      log("Decrypted URL:" + url.replace(encryptedN, decryptedN));
    }

    url = url.replace(encryptedN, decryptedN);
  } else if (doLogging) log("No NParam found in (" + url + ")");
  return url;
}
function decodeCipher(cipher, jsUrl) {
  if (!_cipherDecode[jsUrl])
    throw new ScriptException(
      "Cipher decoder was not available [" + jsUrl + "]"
    );
...
function testCiphers() {
  ...
  for (hash of CIPHER_TEST_HASHES) {
    ...
    try {
      ...
      else testResults.push("CipherTest [" + hash + "]: FAIL");
    } catch (ex) {
  ...
  for (result of testResults) {
    if (result.constructor === Array) console.log(result[0], result[1]);
    else console.log(result);
  }
}
source.testCiphers = testCiphers;
function prepareCipher(jsUrl) {
  if (_cipherDecode[jsUrl]) return false; //_cipherDecode[jsUrl];
  log(
    "New JS Url found: [" +
      jsUrl +
      "], fetching new js (total: " +
      (Object.keys(_cipherDecode).length + 1) +
      ")"
  );
  ...
  try {
    ...
    if (!playerCodeResp.isOk) {
      if (bridge.devSubmit)
        bridge.devSubmit("prepareCipher - Failed to get player js", jsUrl);
    ...
    if (bridge.devSubmit)
      bridge.devSubmit(
        "prepareCipher - Failed to get Cipher due to: " + ex,
        jsUrl
      );
...
function clearCipher(jsUrl) {
  if (_cipherDecode[jsUrl]) _cipherDecode[jsUrl] = undefined;
  if (_nDecrypt[jsUrl]) _nDecrypt[jsUrl] = undefined;
}
function getNDecryptorFunctionCode(code, jsUrl) {
  if (_nDecrypt[jsUrl]) return _nDecrypt[jsUrl];
  const nDecryptFunctionArrNameMatch = REGEX_DECRYPT_N.exec(code);
  if (!nDecryptFunctionArrNameMatch) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getNDecryptorFunctionCode - Failed to find n decryptor (name)",
        jsUrl
      );
  ...
  const nDecryptFunctionNameMatch = code.match(
    nDecryptFunctionArrName + "\\s*=\\s*\\[([a-zA-Z0-9,\\(,\\)\\.]+?)]"
  );
  if (!nDecryptFunctionNameMatch) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getNDecryptorFunctionCode - Failed to find n decryptor (array)",
        jsUrl
      );
  ...
  if (nDecryptArray.length <= nDecryptFunctionArrIndex) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getNDecryptorFunctionCode - Failed to find n decryptor (index)",
        jsUrl
      );
  ...
  const nDecryptFunctionCodeMatch = code.match(
    nDecryptFunctionName + '=function\\(a\\)\\{[\\s\\S]*?join\\(\\"\\"\\)};'
  );
  if (!nDecryptFunctionCodeMatch) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getNDecryptorFunctionCode - Failed to find n decryptor (code)",
        jsUrl,
        code
      );
  ...
  return (
    "(function(){" +
    "var " +
    nDecryptFunctionCodeMatch[0] +
    "\n" +
    "return function decryptN(nEncrypted){ return " +
    nDecryptFunctionName +
    "(nEncrypted); } \n" +
    "})()"
  );
}
function getCipherFunctionCode(playerCode, jsUrl) {
  if (_cipherDecode[jsUrl]) return _cipherDecode[jsUrl];
  ...
  if (!cipherFunctionName) {
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getCipherFunctionCode - Failed to find cipher (name)",
        jsUrl
      );
    throw new ScriptException("Failed to find cipher (name)");
  }
  const cipherFunctionCodeMatch = playerCode.match(
    "(" +
      cipherFunctionName.replace("$", "\\$") +
      "=function\\([a-zA-Z0-9_]+\\)\\{.+?\\})"
  );
  if (!cipherFunctionCodeMatch) {
    ...
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getCipherFunctionCode - Failed to find cipher (function)",
        jsUrl
      );
  ...
  const helperObjNameMatch = cipherFunctionCode.match(
    ";([A-Za-z0-9_\\$]{2,3})\\...\\("
  );
  if (!helperObjNameMatch) {
    if (IS_TESTING) console.log("Failed to find helper name in: ", playerCode);
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getCipherFunctionCode - Failed to find helper (name)",
        jsUrl
      );
    throw new ScriptException("Failed to find helper (name)");
  }
  if (IS_TESTING) console.log("Cipher Code: ", cipherFunctionCode);
  const helperObjName = helperObjNameMatch[1];
  const helperObjMatch = playerCode.match(
    "(var " + escapeRegex(helperObjName) + "=\\{[\\s\\S]*?\\};)"
  );
  if (!helperObjMatch) {
    if (IS_TESTING)
      console.log(
        "Failed to find helper method [" + helperObjName + "] in: ",
        playerCode
      );
    if (bridge.devSubmit)
      bridge.devSubmit(
        "getCipherFunctionCode - Failed to find helper (methods)",
        jsUrl
      );
  ...
  const functionCode =
    "return function decodeCipher(str){ return " +
    cipherFunctionName +
    "(str); }";
...
function parseQueryString(query) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (valueIndex == -1) results[part] = true;
    else
